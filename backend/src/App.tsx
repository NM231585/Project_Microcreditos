...
type Page = 'landing' | 'login' | 'register' | 'dashboard' | 'nueva-solicitud' | 'admin' | 'cronograma';
...
function App() {
    ...
    const mockSolicitudes: Solicitud[] = [
      {
        ...
        datosSolicitud: {
          monto: 5000,
          plazoMeses: 12,
          motivo: 'Ampliación de invernadero y compra de semillas'
        },
        scoreAutomatico: 85,
        cronogramaId: 'cron1'
      },
      {
        ...
        datosSolicitud: {
          monto: 3000,
          plazoMeses: 6,
          motivo: 'Compra de alimento y mejora de instalaciones'
        },
        scoreAutomatico: 78
      }
    ];
  ...
  const generateCuotas = (monto: number, meses: number, tasaAnual: number): Cuota[] => {
  ...
  const handleUpdateSolicitud = (solicitudId: string, updates: Partial<Solicitud>) => {
  ...
  const handleAprobarSolicitud = (solicitudId: string) => {
    ...
    if (solicitud?.datosSolicitud) {
      const cronograma: Cronograma = {
        ...
        tasaInteres: 12,
        cuotas: generateCuotas(solicitud.datosSolicitud.monto, solicitud.datosSolicitud.plazoMeses, 12)
      };
      setCronogramas(prev => [...prev, cronograma]);
      handleUpdateSolicitud(solicitudId, { estado: 'aprobado', cronogramaId: cronograma.id });
    }
  };
  ...
  const handleMarcarPago = (cronogramaId: string, cuotaNumero: number) => {
    setCronogramas(prev => prev.map(c => {
      if (c.id === cronogramaId) {
        return {
          ...c,
          cuotas: c.cuotas.map(cuota => 
            cuota.numero === cuotaNumero ? { ...cuota, pagado: true } : cuota
          )
        };
      }
      return c;
      ...
      case 'dashboard':
        return currentUser ? (
          <EmprendedorDashboard 
            user={currentUser}
            solicitudes={solicitudes.filter(s => s.emprendedorId === currentUser.id)}
      ...
      case 'cronograma':
        ...
        return currentUser && solicitud && cronograma ? (
          <CronogramaView 
            ...
            onBack={() => setCurrentPage(currentUser.rol === 'emprendedor' ? 'dashboard' : 'admin')}
